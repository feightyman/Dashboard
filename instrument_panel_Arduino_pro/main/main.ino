...
{
   // 绘制无线信号图标
   int signalX = 7;  // 信号图标起始 x 坐标
   int signalY = 5;  // 信号图标起始 y 坐标
   int barWidth = 6; // 每格信号宽度
   int barHeight = 40; // 最高格信号高度
   int barGap = 5;   // 格与格之间的间距
...
void draw_meters(){
  ...
  spr_meters.fillSmoothRoundRect(x_start,y_start,63,180,0,TFT_BLACK, TFT_BLACK); 
  ...
  spr_meters.fillRect(30-7.5,  1*51+y_start+55+30+15-(chan11_raw-1000)*30/1000, 15, chan11_raw*30/100, GREEN);
...
void draw_meters_2(){
  ...
  spr_meters_2.fillSmoothRoundRect(x_start,y_start,63,180,0,TFT_BLACK, TFT_BLACK);
...
void draw_new_attitude(){
  ...
  // 箭头两侧点坐标
  float arrow_angle_offset = radians(10);
  ...
  // 绘制箭头
  spr.drawLine(tip_x, tip_y, left_x, left_y, TFT_GREEN);
  spr.drawLine(tip_x, tip_y, right_x, right_y, TFT_GREEN);
  spr.fillTriangle(tip_x, tip_y, left_x, left_y, right_x, right_y, TFT_GREEN);
  ...
  spr.drawWideLine(xb2, yb2, arrow_lx2, arrow_ly2, 2, GREEN);
  ...
  spr.drawWideLine(xb3, yb3, arrow_rx2, arrow_ry2, 2, GREEN);
  ...
  for (int i = 0; i < num_att_lines; i++) {
    ...
    if ((draw_level_angle == 0) && (xb1>1) && (xb1<x_limit) && (xb2>1) && (xb2<x_limit) && (yb1>1) && (yb1<y_limit) && (yb2>1)  && (yb2<y_limit)){
      spr.drawWideLine(xb1, yb1, xb2, yb2, 2, GREEN); //画一根长的地平线
    ...
    if ((draw_level_angle == 0) && (xb3>1) && (xb4>1) && (xb3<x_limit) && (xb4<x_limit) && (yb3>1) && (yb4>1) && (yb3<y_limit) && (yb4<y_limit) ){
      spr.drawWideLine(xb3, yb3, xb4, yb4, 2, GREEN); //画一根长的地平线
...
void setup() {
  ...
  spr.createSprite(150, 180);  //attitude indicator
  ...
  // spr.createSprite(150, 181);  //attitude indicator（横版）
  spr_under.createSprite(320, 60);
