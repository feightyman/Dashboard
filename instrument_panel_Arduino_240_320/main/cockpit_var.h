...
float scale = 1.5;
// float x_limit = 120 * scale; //定义姿态仪画布大小(原版)
// float x_limit = 53 * scale; //定义姿态仪画布大小（竖版）
float x_limit = 100 * scale; //定义姿态仪画布大小（横版）
// float y_limit = 160 * scale; //定义姿态仪画布大小(原版)
float y_limit = 120 * scale;
...
float arc_radius = 80; // 圆弧半径
// 修改姿态线数量
const int num_att_lines = 9; 
//定义存储attitude数据的2维数组
int att_points[num_att_lines][8];

//定义存储上一轮attitude数据的2维数组
int old_att_points[num_att_lines][8];
...
//定义地平线长度 r1----r2  0  r3----r4 按这样的造型编号，数字是从中心出发的像素//（原版）
// float r1 = 30 * scale;
// float r2 = 15 * scale;
// float r3 = 15 * scale;
// float r4 = 30 * scale;
...
float l3 = 5 * scale;
float l4 = 35 * scale;
float l0 = 80 * scale; // 用于绘制地面天空的分界线
const float level_pixel = 15 * scale*0.8; //各水平姿态线距离pix,距离扩大1.5倍，方便显示

const int level_angle = 5; //定义各水平姿态线距离角度为5度
...
// #define MAVLINK_MSG_ID_RC_CHANNELS_RAW 35
 ...
 uint16_t chan13_raw; ///< RC channel 13 value, in microseconds
 uint16_t chan14_raw; ///< RC channel 14 value, in microseconds
 uint16_t chan15_raw; ///< RC channel 15 value, in microseconds
 uint16_t chan16_raw; ///< RC channel 16 value, in microseconds
 uint16_t chan17_raw; ///< RC channel 17 value, in microseconds
 uint16_t chan18_raw; ///< RC channel 18 value, in microseconds
...
const unsigned long blinkInterval = 350; // 闪烁间隔，单位：毫秒
bool blinkState = false; // 闪烁状态

// 定义闪烁相关变量
unsigned long blinkMillisWifi = 0;
bool blinkStateWifi = false;
const unsigned long blinkIntervalWifi = 500; // 闪烁间隔，单位为毫秒

// 全局变量存储起飞点坐标（需在初始化时设置）
int32_t home_lat = 0;  // 起飞点纬度（十进制度）
int32_t home_lon = 0;  // 起飞点经度（十进制度）
bool first_arm=true;
double distance;

// 全局状态变量（volatile保证主循环与中断间的可见性）
volatile double total_distance = 0;     // 总路程（米）
volatile uint32_t last_time_stamp = 0;  // 上次时间戳
volatile float last_groundspeed = 0;    // 上次地速
