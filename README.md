...
2.  **`v2.0-lite` (轻量化 / 低配分支)** *(注：请替换为您实际的分支名)*
      * **特点**: 裁剪了部分复杂的渐变和高清位图贴图，优化了内存占用（RAM）和 CPU 开销。
      * **适用硬件**: 适用于经典单片机如 Arduino Mega 2560 或低主频 ARM 芯片。
3.  **`v1.0-legacy` (经典旧版 / 稳定分支)** *(注：请替换为您实际的分支名)*
      * **特点**: 项目早期的稳定版本，保留了最基础的地平线逻辑和单色/简单色彩 UI 风格，代码结构最简单，适合新手学习和二次开发。
